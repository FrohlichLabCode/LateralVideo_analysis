...
for iCond = condID
    ...
    for iev = 1:numel(event)
        ...
        ymat(iev,:,:,:) = yZpow(:,:,evSamp+tsamps(1):evSamp+tsamps(2));
        zmat(iev,:,:,:) = zZpow(:,:,evSamp+tsamps(1):evSamp+tsamps(2));
        xBasemat(iev,:,:,:) = xZpow(:,:,evSamp+baseSamps(1):evSamp+baseSamps(2));
        yBasemat(iev,:,:,:) = yZpow(:,:,evSamp+baseSamps(1):evSamp+baseSamps(2));
        zBasemat(iev,:,:,:) = zZpow(:,:,evSamp+baseSamps(1):evSamp+baseSamps(2));
    end
    
    spectra.tvec  = (tsamps(1):tsamps(2))/fs;
    spectra.foi   = foi;
    spectra.xspec = squeeze(nanmean(nanmean(xmat,1),3)); %mean over events and channel
    spectra.yspec = squeeze(nanmean(nanmean(ymat,1),3));
    spectra.zspec = squeeze(nanmean(nanmean(zmat,1),3));

    % take mean over event and time, get an array of trial means: 1 x f x channel x 1
    xBaseMeanVec = squeeze(nanmean(nanmean(xBasePow,1),4));
    xBaseStdVec  = squeeze(nanstd( nanmean(xBasePow,4),1));
    yBaseMeanVec = squeeze(nanmean(nanmean(yBasePow,1),4));
    yBaseStdVec  = squeeze(nanstd( nanmean(yBasePow,4),1));
    zBaseMeanVec = squeeze(nanmean(nanmean(zBasePow,1),4));
    zBaseStdVec  = squeeze(nanstd( nanmean(zBasePow,4),1));
    
    % broadcast to same dimension: event x trial x time
    xBaseMean    = repmat(xBaseMeanVec,[numel(event),1,numSampsWin]);
    xBaseStd     = repmat(xBaseStdVec, [numel(event),1,numSampsWin]);
    yBaseMean    = repmat(yBaseMeanVec,[numel(event),1,numSampsWin]);
    yBaseStd     = repmat(yBaseStdVec, [numel(event),1,numSampsWin]);
    zBaseMean    = repmat(zBaseMeanVec,[numel(event),1,numSampsWin]);
    zBaseStd     = repmat(zBaseStdVec, [numel(event),1,numSampsWin]);
    
    spectra.xspecNormed = squeeze(nanmean((xPow-xBaseMean)./xBaseStd,1));
    spectra.yspecNormed = squeeze(nanmean((yPow-yBaseMean)./yBaseStd,1));
    spectra.zspecNormed = squeeze(nanmean((zPow-zBaseMean)./zBaseStd,1));
    ...
    save([saveAnalysisDir 'LateralVideoSpectra_' condNames{iCond} '.mat'],'foi','tvec','powLvideo_pul','powLvideo_ppc','powLvideo_vc','powRvideo_pul','powRvideo_ppc','powRvideo_vc','-v7.3');
